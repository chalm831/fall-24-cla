...
#this program lets you change the format of the image 
...
# img = Image.open( sys.argv[1] )
# one_pixel = img.getpixel( (0,0) ) #variable image get pixel the most top left dot in any image, calling the coordinate in paranthesis so you wrap them in parenthesis 

# print(one_pixel) #returning a pixel

# img.putpixel( (10,10), (255,0,0) ) #this is coordinate and then the color HEX value RGB
# img.save("new.png") #this saves the image as a new image
#pngs doesnt blend the color in so its easier to see than a jpg in terms of change doen to the image
...
# img = Image.open( sys.argv[1] )
# img_hsv = img.convert(mode="HSV") #makeing it into another color space 
# img_hsv_data = img_hsv.getdata()

# new_img_data = [] #this is an empty value remmmmbrrr empty list 
# for p in img_hsv_data: # for x in the data 
#     print(p) 
#     if p[2] < 55: #if the 2nd value which is saturation is less then 55 
#         new_img_data.append( (255,255,255) ) # im going to add that to my empty list 
#     else:
#         new_img_data.append(p) # let the regular pixel stay 


# img_hsv.putdata(new_img_data) #pu the data into the image
# img_rgb = img_hsv.convert("RGB") #turning into rgb 
# img_rgb.save("filtered.jpg") #saving
...
# data = []
# for i in range(160000): #for all of those pixels in that range
#     pixel = (i, 0, 255-i)
#     data.append( pixel )
